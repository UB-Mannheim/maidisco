...
PRIMO_SEARCH_ENDPOINT = os.environ.get(
...
def call_primo_search(params):
    ...
    try:
        r = requests.get(
            PRIMO_SEARCH_ENDPOINT, params=params, headers=headers, timeout=15
        )
        r.raise_for_status()
        return r.json()
...
def index():
    return render_template_string(
        INDEX_HTML,
        example="e.g. Recent articles (2019-2024) on climate resilience in urban planning, English, peer-reviewed",
        query=None,
    )
